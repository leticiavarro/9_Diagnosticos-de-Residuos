...
install.packages("lmtest")     #instalar pacotes
install.packages("aTSA")
library(lmtest)                #carrega o pacote
library(aTSA)
...
View(jocci)        # abrir a tabela
JOCCI <- as.data.frame(jocci) # transformando os dados em um dado frame em forma de tabela                                    #Y é o Index Jocci . dy é a variação (expressa pela diferença) em logaritmos.
joccits <- ts(JOCCI$dy,start = 1959,frequency = 12)   # transformar os dados da tabela que o DY são os dados univariaveis (série temporal)
plot(joccits, main="Índice Jocci", xlab="Ano", ylab="Indice") # fazer grafico e mudar o títutlo
...
TesteBGAR6 <- bgtest(ar6model,data=JOCCI)    # comando para fazer o teste
...
TesteBGAR1 <- bgtest(ar1model,data=JOCCI)   # criou o objeto e salvou

P_Valores_BG <- c(TesteBGAR6$p.value,    #criando um vetor     
               TesteBGAR5$p.value,
               TesteBGAR4$p.value,
               TesteBGAR3$p.value,
               TesteBGAR2$p.value,
